...
extension OctoDog {
    struct Projects {
        ...
        func reviewUserPermissionLevel() -> Response<ReviewAUserSPermissionLevelResponse> {
            let data = Data()
            do {
                let decoder = JSONDecoder()
                let body = try decoder.decode(ReviewAUserSPermissionLevelResponse.self, from: data)
                return Response(body: body, error: nil)
