...
///Note that you'll need to set `Content-Length` to zero when calling out to this endpoint. For more information, see "[HTTP verbs](https://developer.github.com/v3/#http-verbs)."
      func publicizeMembership() -> Response<PublicizeAUser'SMembershipResponse> {
      
        let data = Data()
        do {
          let decoder = JSONDecoder()
          let body = try decoder.decode(PublicizeAUser'SMembershipResponse.self, from: data)
          return Response(body: body, error: nil)
      ...
      /// 
      func concealMembership() -> Response<ConcealAUser'SMembershipResponse> {
      
        let data = Data()
        do {
          let decoder = JSONDecoder()
          let body = try decoder.decode(ConcealAUser'SMembershipResponse.self, from: data)
          return Response(body: body, error: nil)
