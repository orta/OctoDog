...
extension OctoDog {
    struct CodesOfConduct {
        ...
        func getForRepo() -> Response<GetTheContentsOfARepositorySCodeOfConductResponse> {
            let data = Data()
            do {
                let decoder = JSONDecoder()
                let body = try decoder.decode(GetTheContentsOfARepositorySCodeOfConductResponse.self, from: data)
                return Response(body: body, error: nil)
