...
extension OctoDog {
    struct Gists {
        ...
        // https://developer.github.com/v3/gists/#list-a-users-gists
        ///
        func list() -> Response<ListTheAuthenticatedUserSGistsOrIfCalledAnonymouslyThisWillReturnAllPublicGistsResponse> {
            let data = Data()
            do {
                let decoder = JSONDecoder()
                let body = try decoder.decode(ListTheAuthenticatedUserSGistsOrIfCalledAnonymouslyThisWillReturnAllPublicGistsResponse.self, from: data)
                return Response(body: body, error: nil)
