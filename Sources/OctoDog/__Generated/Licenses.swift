...
extension OctoDog {
    struct Licenses {
        ...
        func getForRepo() -> Response<GetTheContentsOfARepositorySLicenseResponse> {
            let data = Data()
            do {
                let decoder = JSONDecoder()
                let body = try decoder.decode(GetTheContentsOfARepositorySLicenseResponse.self, from: data)
                return Response(body: body, error: nil)
